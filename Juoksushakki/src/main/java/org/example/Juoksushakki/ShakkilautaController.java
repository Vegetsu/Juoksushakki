...
public class ShakkilautaController {
    ...
    private MouseButton stealButton;
    boolean stealCompleted = false;
    private boolean stealTurnActive = false;
    private Pelintallennus pelintallennus;
    /*
   ...
   public void initialize() {
       gridPane.layoutBoundsProperty().addListener((obs, oldVal, newVal) -> drawLines());
 ...
 */
        ...
        if (success) {
             ...
             */
                ...
                updateMoveHistory();
                if (!board.getSiirtohistoria().isEmpty()) {
                    System.out.println("Siirtohistoria: " + board.getSiirtohistoria());
                    pelintallennus.muistiinLiike(
                            board.getSiirtohistoria().get(
                                    board.getSiirtohistoria().size() - 1
                            )
                    );
                }
            ...
            if (stealCompleted) {
                ...
                if (stealButton == MouseButton.PRIMARY) {
                    setWhiteTurn();
                    activeButton = MouseButton.PRIMARY;
                } else {
                    setBlackTurn();
                ...
                stealTurnActive = true;
                stealCompleted = false;

            } else {

                if (stealTurnActive) {
                    stealTurnActive = false;
                }
   ...
   mutta sitä pystyy kuitenkin liikuttamaan pois shakkilaudan edestä.
    ...
    private void avaagameover() throws IOException {
        ...
        if (board.getCurrentTurn().equals("black")) {
            pelintallennus.kirjoitaLopputulos(
                    "Voittaja: " + whitePlayerName + " (Valkoinen)"
            );
        } else {
            pelintallennus.kirjoitaLopputulos(
                    "Voittaja: " + blackPlayerName + " (Musta)"
            );
        }
 ...
 */
    ...
    public void startNewGame() {
        try {
            ...
            pelintallennus.kirjoitaLopputulos(
                    "Peli keskeytettiin: uusi peli aloitettiin."
            );
            pelintallennus.close();
        } catch (IOException e) {
            e.printStackTrace();
...
private void drawLines() {
    ...
    /*
    Annetaan yhteys Pelintallennus-luokan metodeihin, jotta pelin siirtohistoria ja muut olennaiset tiedot voidaan tallentaa erilliselle
    tekstitiedostolle.
     */
    public void setPelintallennus(Pelintallennus pelintallennus) {
        this.pelintallennus = pelintallennus;
    }
