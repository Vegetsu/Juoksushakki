...
public class ShakkilautaModel {
    ...
    private void setupPieces() {
        for (int i = 0; i < 8; i++) {
            board[6][i] = new Soturi("white");
        }

        for (int i = 0; i < 8; i++) {
            board[1][i] = new Soturi("black");
        }
...
/*
Tarkistetaan, onko potentiaalinen liike tornitus-liike, ja jos on niin suoritetaan liike ja lisätään se siirtohistoriaan.
 ...
 */
        if (isCastlingMove(r1, c1, r2, c2)) {
            ...
            int moveNumber = siirtohistoria.size() / 2 + 1;
        ...
        /*
Tarkistetaan, onko potentiaalinen liike enpassant-liike, ja jos on niin suoritetaan liike ja lisätään se siirtohistoriaan.
Lopuksi vaihdetaan vuoro.
 */
        if (isEnPassantMove(r1, c1, r2, c2)) {

            Piece soturi = board[r1][c1];

            board[r2][c2] = soturi;
            board[r1][c1] = null;


            int dir = soturi.getColor().equals("white") ? 1 : -1;
            ...
            soturi.setHasMoved(true);

            String color = soturi.getColor().equals("white") ? "Valkoinen" : "Musta";
            int moveNumber = siirtohistoria.size() / 2 + 1;
            historyAlreadyAdded = true;
            String move = moveNumber + ". " + color + " Soturi: "
                    + "En passant "
        ...
        /*
        Asetetaan true ja false arvoja tornituksessa mukana olevilla nappuloille, jotta niiden korostus toimii oikein
        tornitustilanteissa.
         */
        ...
        /*
        Tarkistetaan liikkeen tapahduttua, että jääkö siirron tehneen pelaajan kuningas shakkiin siirron takia ja jos jää,
        niin perutaan liike.
         */
        if (isKingInCheck(source.getColor())) {
            undoMove(r1, c1, r2, c2, target);
            return false;
 ...
 */
    ...
    /*
    Metodi, jolla tarkastetaan, että onko valitulla nappulalla yhtään sallittuja siirtoja. Koko ruutu käydään läpi ja
    kaikki mahdolliset siirrto simuloidaan kaikille nappuloille, jonka jälkeen palautetaan, että onko liike sallittu vai ei.
     */
    ...
    /*
Tarkistaa, että onko tilanne shakkimatti.
     */
    ...
    /*
    Tarkistaa, että onko pattitilanne
     */
    ...
    /*
    Etsii halutun värisen kuninkaan laudalta.
     */
    ...
    /*
    Tarkistaa, että onko kuningas shakissa. Tallentaa kuninkaan sijainnin, jonka jälkeen verrataan kaikkien vastustajan
    nappuloiden sallittuja liikkeitä ja katsotaan, shakittavatko ne mikään tarkistuksen alla olevan kuninkaan.
     */
 ...
 */
    ...
    /*
    Metodi, jonka avulla soturi korotetaan. Pelaaja saa valinnan neljästä vaihtoehdosta, jonka jälkeen soturi korotetaan
    ...
    /*
    Metodi, jolla tarkastetaan, että onko jokin ruutu hyökkäysuhan alla. Käytetään tornituksen tarkastuksessa.
     */
    ...
    /*
    Tarkastetaan, että onko yritettävä siirto tornitus-siirto. Siirrettävä nappula pitää olla kuningas ja sen pitää
    ...
    /*
    Tarkastetaan, että onko tornitus sallittu liike.
     */
    public boolean canCastle(
        ...
        /*
        Lyhyt siirto, torni ei saa olla liikkunut  ja kuninkaan ja tprnin välissä olevien ruutujen täytyy olla tyhjiä
        ...
        /*
        Pitkä siirto, torni ei saa olla liikkunut  ja kuninkaan ja tprnin välissä olevien ruutujen täytyy olla tyhjiä
        eivätkä ne saa olla hyökkäyksen uhan alla.
         */
        if (c2 == 2) {
    ...
    /*
    Tornitusliike, jos lyhyt siirto niin siirretään torni kuninkaan vasemmalle puolelle samalla kun kuningasta liikutetaan
    ...
    /*
Tarkistaa, että onko liike enpassant-liike. Oman soturin täytyy olla liikkunut kolme ruutua lähtöruudustaan ja vastustajan
soturin täytyy hypätä kaksi ruutua eteenpäin oman soturin viereen. Sitten tarkastetaan, että vastustajan soturin takana
 ...
 */
    public List<int[]> getLegalMoves(int row, int col) {
        ...
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                ...
                if (safe && isValidMove(row, col, r, c)) {
        ...
        /*
        Jos kyseinen nappula on soturi, niin tarkastetaan lisäksi, että onko enpassant-liike sille mahdollinen.
         */
        if (piece instanceof Soturi) {
        ...
        /*
        Jos kyseinen nappula on kuningas, niin tarkastetaan lisäksi, että onko tornitus sille mahdollinen.
         */
    ...
    /*
    Tarkistaa, että onko enpassant-liike mahdollista suorittaa. Vastustajan viimeksi siirretty nappula täytyy olla sotilas
    ...
    public boolean canEnPassant(int r1, int c1, int r2, int c2) {
        ...
        if (!(lastMovedPiece instanceof Soturi))
            return false;
    ...
    /*
Tarkistaa, että voiko kuninkaan puolelle suorittaa lyhyen tornituksen. Samat tarkastukset kuin mitä aiemmin on jo kerrottu
tornitukseen liittyen.
     */
    ...
    /*
Tarkistaa, että voiko kuningattaren puolelle suorittaa pitkän tornituksen. Samat tarkastukset kuin mitä aiemmin on jo kerrottu
tornitukseen liittyen.
     */
    ...
    /*
    Käy läpi kaikki vastustajan nappulat ja laittaa listaan ne, jotka ovat shakittamassa kunigasta.
     */
    public List<int[]> findCheckingPieces(String kingColor) {
        ...
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                ...
                if (p == null) continue;
                if (!p.getColor().equals(opponent)) continue;
